...
#space complexity O(n)
# initialize the recursive function that takes in an index
# check if the index is greater than or equal to the length of the cost array
# if so, return 0 since we are at the end of the array
# return the cost of the current index plus the min of the next two indices
#outside function
# return the minimum of the two recursive calls starting at index 0 and 1, the two possible starting points
class Solution:
    def minCostClimbingStairs(self, cost: List[int]) -> int:
        def step(index):
            if index >= len(cost):
                return 0
            return cost[index] + min(step(index + 1), step(index + 2))
